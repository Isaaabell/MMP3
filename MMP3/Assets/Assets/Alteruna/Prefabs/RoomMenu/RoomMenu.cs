...
namespace Alteruna
{
	public class RoomMenu : CommunicationBridge
	{
		...
		[SerializeField] private CanvasGroup _roomMenuCanvasGroup;
		...
		private void Start()
		{
			_roomMenuCanvasGroup.alpha = 1;
			_roomMenuCanvasGroup.interactable = true;
			_roomMenuCanvasGroup.blocksRaycasts = true;
			...
			else
			{
				...
				StartButton.onClick.AddListener(() =>
				{
					...
					Debug.Log("StartButton pressed: Attempting to join an on-demand room.");
				});
		...
		private void JoinedRoom(Multiplayer multiplayer, Room room, User user)
		{
			...
			_roomMenuCanvasGroup.alpha = 0;
			_roomMenuCanvasGroup.interactable = false;
			_roomMenuCanvasGroup.blocksRaycasts = false;
			...
			for (int i = 0; i < multiplayer.AvailableRooms.Count; i++)
			{
				...
				if (_roomObjects.Count > i)
				{
					...
					else
					{
						...
						Debug.Log($"Room '{room.Name}' join button pressed. ID: {room.ID}");
